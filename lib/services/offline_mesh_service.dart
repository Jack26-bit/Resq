...
class OfflineMeshService {
  ...
  Future<void> queueSosReport(String id, Map<String, dynamic> data) async {
    if (!supportsBluetoothMesh) {
      debugPrint(
          'Nearby mesh is unavailable on this platform; skipping offline SOS broadcast.');
      return;
    }
        ...
        onConnectionResult: (id, status) {
          if (status == Status.CONNECTED) {
            Nearby().sendBytesPayload(
                id, Uint8List.fromList(utf8.encode(jsonEncode(data))));
          }
        },
        onDisconnected: (id) {},
      );
  ...
  Future<void> syncOfflineData() async {
    ...
    final List<Map<String, dynamic>> queued =
        await _db?.query('sos_queue') ?? [];
    for (var row in queued) {
      try {
        final payload = jsonDecode(row['payload'] as String);
        payload['timestamp'] =
            FieldValue.serverTimestamp(); // Refresh timestamp
