...
class _SosScreenState extends State<SosScreen>
              ...
              child: Column(
                children: [
                  ...
                  Row(
                    children: [
                      Expanded(
                          child: _mediaBtn(Icons.photo_camera, 'Add Images', _captureImage)),
                      const SizedBox(width: 12),
                      Expanded(
                          child: _mediaBtn(Icons.videocam, 'Record Video', _captureVideo)),
                    ],
                  ),
  ...
  Future<void> _captureImage() async {
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Error capturing image: $e', style: const TextStyle(color: Colors.white)), backgroundColor: C.error));
      }
    }
  }
  ...
  Future<void> _captureVideo() async {
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Error capturing video: $e', style: const TextStyle(color: Colors.white)), backgroundColor: C.error));
      }
    }
  }
  ...
  Future<void> _submitSOS() async {
    ...
    try {
      final docData = {
        ...
        'location': {
          'latitude': _currentPosition?.latitude ?? 34.0522, // Fallback to mocked for MVP if null
          'longitude': _currentPosition?.longitude ?? -118.2437,
        },
        'media_attachments': _mediaFiles.map((f) => f.name).toList(),
