...
class _SmartScanScreenState extends State<SmartScanScreen>
  ...
  Future<void> _initCamera() async {
    try {
      ...
      setState(
          () => _statusText = 'CAMERA ERROR: ${e.toString().toUpperCase()}');
    }
  }
  ...
  void _startScanning() {
    ...
    try {
      _camCtrl!.startImageStream((image) async {
        ...
        if (items.isNotEmpty) {
          ...
          for (final item in items) {
            ...
            final exists = _scanLog
                .any((r) => r.tag == tag && (r.confidence - conf).abs() < 0.1);
            if (!exists) {
      ...
      setState(() {
        _isScanning = false;
        _statusText =
            'SCAN ERROR: LIVE SCANNING NOT SUPPORTED ON THIS PLATFORM';
      });
  ...
  Future<void> _stopScanning() async {
    ...
    // On web, stopImageStream might throw or hang if the platform doesn't support it well.
    // We update state first to keep the UI responsive.
    setState(() {
      _isScanning = false;
    ...
    try {
      ...
      debugPrint(
          'Note: stopImageStream failed (expected on some web browsers): $e');
    }
  }
  ...
  Future<void> _captureAndAnalyse() async {
    ...
    try {
      ...
      setState(() => _statusText = 'RUNNING GEMINI AI ANALYSIS...');
      final bytes = await file.readAsBytes();
      final gemini = GeminiService();
      final aiSummary = await gemini.analyzeImage(bytes, 'image/jpeg');

      _scanLog.add(_ScanResult(
        tag: 'Gemini AI Analysis',
        mlLabel: aiSummary,
        confidence: 1.0,
        time: DateTime.now(),
      ));
              ...
              child: Container(
                ...
                child: const Row(
                  children: [
                    ...
                    Expanded(
                      child: Text(
                        'WEB LIMITATION: ML Kit Object Detection is only supported on Android/iOS. Live scanning will not detect objects on Web.',
                        style: TextStyle(
                            color: Colors.white,
                            fontSize: 12,
                      ...
                      Row(
                        children: [
                          ...
                          Expanded(
                            child: GestureDetector(
                              onTap:
                                  _scanLog.isNotEmpty ? _returnResults : null,
                              child: Container(
                                padding:
                                    const EdgeInsets.symmetric(vertical: 16),
                                decoration: BoxDecoration(
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (final det in detections) {
      ...
      if (det.trackingId != null) {
        ...
        idPainter.paint(
            canvas, Offset(rect.right - idPainter.width - 4, rect.bottom + 4));
      }
    }
  }
